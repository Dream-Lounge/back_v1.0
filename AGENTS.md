...
│   │   ├── auth_service.py      # 회원가입 처리, 인증 세션 연결
...
2. 입력 항목: 학번 10자리, 숫자 4자리 비밀번호
3. 회원가입 완료 후 로그인 화면으로 이동
- **로그인**: 학번 + 비밀번호
...
등록 항목:
- 동아리 공개 연락망: 이메일, 오픈채팅 링크, 전화번호 (텍스트 입력)
...
### 개발 순서
1. **인증** — 학번 회원가입, 로그인 (HttpOnly 쿠키 세션)
...
| POST | `/api/v1/auth/login` | 로그인 → HttpOnly 쿠키 세션 발급 |
